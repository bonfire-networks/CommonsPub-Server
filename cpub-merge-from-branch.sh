...
for file in cpub-merge-from-upstream.sh cpub-merge-from-branch.sh README.md DEPLOY.md HACKING.md config/docker.env config/docker.dev.env Makefile docker-compose.yml docker-compose.pi.yml .gitlab-ci.yml lib/moodle_net_web/graphql/schema.ex lib/value_flows priv/repo/migrations/*_vf_*
do
    git reset HEAD ${file}
    git checkout -- ${file}
done
...
if echo "$answer" | grep -iq "^c" ; then
    ...
    git commit -m "merged from ${FROM_BRANCH}"
    git push
else 
