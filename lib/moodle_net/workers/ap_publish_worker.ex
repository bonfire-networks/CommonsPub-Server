...
defmodule MoodleNet.Workers.APPublishWorker do
  ...
  def perform(%{"context_id" => context_id}, _job) do
    ...
    {:ok, _} = publish(context)
  end

  defp publish(%Comment{} = comment), do: ignore_deleted(comment, &Publisher.comment/1)
  defp publish(%Resource{} = resource), do: ignore_deleted(resource, &Publisher.create_resource/1)
  defp publish(%Community{} = community), do: ignore_deleted(community, &Publisher.create_community/1)
  defp publish(%Collection{} = collection), do: ignore_deleted(collection, &Publisher.create_collection/1)
  defp publish(%Flag{} = flag), do: ignore_deleted(flag, &Publisher.flag/1)
  ...
  defp publish(%{__struct__: type}) do
    Logger.warn("Unsupported type for AP publisher: #{to_string(type)}")
