...
defmodule MoodleNet.Activities.Queries do
  ...
  def join_to(q, :context, jq), do: join(q, jq, [activity: a], c in assoc(a, :context), as: :context)
  ...
  def filter(q, {:published, {:gte, %DateTime{}=time}}), do: where(q, [activity: a], a.published_at >= ^time)
  def filter(q, {:published, {:lte, %DateTime{}=time}}), do: where(q, [activity: a], a.published_at <= ^time)
  ...
  def filter(q, {:deleted, {:gte, %DateTime{}=time}}), do: where(q, [activity: a], a.deleted_at >= ^time)
  def filter(q, {:deleted, {:lte, %DateTime{}=time}}), do: where(q, [activity: a], a.deleted_at <= ^time)
  ...
  def filter(q, {:creator, id}) when is_binary(id), do: where(q, [activity: a], a.creator_id == ^id)
  def filter(q, {:creator, ids}) when is_list(ids), do: where(q, [activity: a], a.creator_id in ^ids)

  def filter(q, {:context, id}) when is_binary(id), do: where(q, [activity: a], a.context_id == ^id)
  def filter(q, {:context, ids}) when is_list(ids), do: where(q, [activity: a], a.context_id in ^ids)
  ...
  def filter(q, {:distinct, [desc: key]}) when is_atom(key), do: distinct(q, [activity: a], [desc: field(a, ^key)])
  def filter(q, {:distinct, [asc: key]}) when is_atom(key), do: distinct(q, [activity: a], [asc: field(a, ^key)])
  def filter(q, {:distinct, key}) when is_atom(key), do: distinct(q, [activity: a], field(a, ^key))
