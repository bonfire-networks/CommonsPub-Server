...
defmodule MoodleNet.Collections.Queries do
  ...
  defp join_to(q, :community, jq), do: join(q, jq, [collection: c], assoc(c, :community), as: :community)
  ...
  def filter(q, {:deleted, {:gte, %DateTime{}=time}}), do: where(q, [collection: c], c.deleted_at >= ^time)
  def filter(q, {:deleted, {:lte, %DateTime{}=time}}), do: where(q, [collection: c], c.deleted_at <= ^time)
  ...
  def filter(q, {:disabled, {:gte, %DateTime{}=time}}), do: where(q, [collection: c], c.disabled_at >= ^time)
  def filter(q, {:disabled, {:lte, %DateTime{}=time}}), do: where(q, [collection: c], c.disabled_at <= ^time)
  ...
  def filter(q, {:published, {:gte, %DateTime{}=time}}), do: where(q, [collection: c], c.published_at >= ^time)
  def filter(q, {:published, {:lte, %DateTime{}=time}}), do: where(q, [collection: c], c.published_at <= ^time)
  ...
  def filter(q, {:order, [asc: :followers]}) do
    order_by q, [collection: c, follower_count: fc],
      asc: coalesce(fc.count, 0), desc: c.id
  end

  def filter(q, {:order, [desc: :followers]}) do
    order_by q, [collection: c, follower_count: fc],
      desc: coalesce(fc.count, 0), desc: c.id
  end
  ...
  def filter(q, {:count, key}) when is_atom(key), do: select(q, [collection: c], {field(c, ^key), count(c.id)})
