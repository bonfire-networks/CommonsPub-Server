...
defmodule CommonsPub.Access.RegisterEmailDomainAccessesQueries do
  alias CommonsPub.Access.RegisterEmailDomainAccess
  import Ecto.Query
  ...
  def filter(q, {:domain, domain}) when is_binary(domain),
    do: where(q, [reda: r], r.domain == ^domain)

  def filter(q, {:domain, domains}) when is_list(domains),
    do: where(q, [reda: r], r.domain in ^domains)

  def filter(q, {:count, key}) when is_atom(key),
    do: select(q, [reda: r], {field(r, ^key), count(r.id)})
  ...
  def filter(q, {:page, [desc: [created: %{limit: l}]]}),
    do: filter(q, limit: l + 1, order: [desc: :created])
