...
defmodule CommonsPub.Threads.CommentsQueries do
  ...
  def filter(q, {:deleted, {:gte, %DateTime{} = time}}),
    do: where(q, [comment: c], c.deleted_at >= ^time)
  ...
  def filter(q, {:hidden, {:gte, %DateTime{} = time}}),
    do: where(q, [comment: c], c.hidden_at >= ^time)
  ...
  def filter(q, {:published, {:gte, %DateTime{} = time}}),
    do: where(q, [comment: c], c.published_at >= ^time)
  ...
  def filter(q, {:in_reply_to, id}) when is_binary(id),
    do: where(q, [comment: c], c.in_reply_to_id == ^id)

  def filter(q, {:in_reply_to, ids}) when is_list(ids),
    do: where(q, [comment: c], c.in_reply_to_id in ^ids)

  def filter(q, {:creator, id}) when is_binary(id),
    do: where(q, [comment: c], c.creator_id == ^id)

  def filter(q, {:creator, ids}) when is_list(ids),
    do: where(q, [comment: c], c.creator_id in ^ids)
  ...
  def filter(q, {:count, key}) when is_atom(key),
    do: select(q, [comment: c], {field(c, ^key), count(c.id)})
