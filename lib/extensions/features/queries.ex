...
defmodule CommonsPub.Features.Queries do
  ...
  defp join_to(q, :context, jq),
    do: join(q, jq, [feature: f], c in assoc(f, :context), as: :context)
  ...
  def filter(q, {:deleted, {:gte, %DateTime{} = time}}),
    do: where(q, [feature: f], f.deleted_at >= ^time)
  ...
  def filter(q, {:context, id}) when is_binary(id),
    do: where(q, [feature: f], f.context_id == ^id)

  def filter(q, {:context, ids}) when is_list(ids),
    do: where(q, [feature: f], f.context_id in ^ids)
  ...
  def filter(q, {:count, key}) when is_atom(key),
    do: select(q, [feature: f], {field(f, ^key), count(f.id)})
