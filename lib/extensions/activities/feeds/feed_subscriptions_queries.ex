...
defmodule CommonsPub.Feeds.FeedSubscriptionsQueries do
  ...
  def filter(q, {:deleted, {:gte, %DateTime{} = time}}),
    do: where(q, [subscription: s], s.deleted_at >= ^time)
  ...
  def filter(q, {:disabled, {:gte, %DateTime{} = time}}),
    do: where(q, [subscription: s], s.disabled_at >= ^time)
  ...
  def filter(q, {:activated, :not_nil}),
    do: where(q, [subscription: s], not is_nil(s.activated_at))
  ...
  def filter(q, {:feed, ids}) when is_list(ids),
    do: where(q, [subscription: s], s.feed_id in ^ids)

  def filter(q, {:subscriber, id}) when is_binary(id),
    do: where(q, [subscription: s], s.subscriber_id == ^id)

  def filter(q, {:subscriber, ids}) when is_list(ids),
    do: where(q, [subscription: s], s.subscriber_id in ^ids)
