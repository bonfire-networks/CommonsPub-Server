...
defmodule CommonsPub.Feeds.FeedActivitiesQueries do
  ...
  defp join_to(q, :activity, jq),
    do: join(q, jq, [feed_activity: fa], a in assoc(fa, :activity), as: :activity)

  defp join_to(q, :context, jq),
    do: join(q, jq, [activity: a], c in assoc(a, :context), as: :context)

  defp join_to(q, :feed, jq),
    do: join(q, jq, [feed_activity: fa], f in assoc(fa, :feed), as: :feed)
  ...
  def filter(q, {:id, {:gte, id}}) when is_binary(id),
    do: where(q, [feed_activity: fa], fa.id >= ^id)
  ...
  def filter(q, {:activity, id}) when is_binary(id),
    do: where(q, [feed_activity: fa], fa.activity_id == ^id)

  def filter(q, {:activity, ids}) when is_list(ids),
    do: where(q, [feed_activity: fa], fa.activity_id in ^ids)
