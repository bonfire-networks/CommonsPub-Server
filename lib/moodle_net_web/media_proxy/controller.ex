defmodule MoodleNetWeb.MediaProxy.MediaProxyController do
  ...
  def remote(conn, %{"sig" => sig, "url" => url}) do
    ...
    with true <- Keyword.get(config, :enabled, false),
         {:ok, url} <- MoodleNetWeb.MediaProxy.decode_url(sig, url),
  ...
  defp proxy_request(link) do
    headers = [
      {"user-agent",
       "MoodleNet/MediaProxy; #{MoodleNetWeb.base_url()} <#{
         Application.get_env(:moodle_net, :instance)[:email]
       }>"}
    ]
