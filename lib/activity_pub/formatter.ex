defmodule MoodleNet.Formatter do
  ...
  @emoji_from_globs (
                      static_path = Path.join(:code.priv_dir(:moodle_net), "static")

                      globs =
                        Application.get_env(:moodle_net, :emoji, [])
  ...
  def add_links({subs, text}) do
    additionnal_schemes =
      Application.get_env(:moodle_net, :uri_schemes, [])
    ...
    subs =
      subs ++
        Enum.map(tags, fn {tag_text, tag, uuid} ->
          url = "<a href='#{MoodleNetWeb.base_url()}/tag/#{tag}' rel='tag'>#{tag_text}</a>"
          {uuid, url}
        end)
