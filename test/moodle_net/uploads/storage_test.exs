...
defmodule MoodleNet.Uploads.StorageTest do
  ...
  describe "store" do
    test "stores a file using a path" do
      assert {:ok, file_info} = Storage.store(@image_path)
      assert file_info.id
    ...
    test "stores a file using a plug upload" do
      assert {:ok, file_info} = Storage.store(%{path: @image_path, filename: "150.png"})
    end
  ...
  describe "remote_url" do
    test "returns a valid URL for an existing identifier" do
      assert {:ok, %{id: file_id}} = Storage.store(@image_path)
