...
defmodule MoodleNet.AccessTest do
  ...
  describe "MoodleNet.Access.is_register_whitelisted?/1" do
    test "returns true when the email's domain is whitelisted" do
      Repo.transaction(fn ->
        ...
        assert true == Access.is_register_whitelisted?(email)
      end)
    end
    ...
    test "returns false when no whitelist pertains to the email" do
      Repo.transaction(fn ->
        assert false == Access.is_register_whitelisted?(Fake.email())
      end)
    end
  end
