...
defmodule CommonsPub.Meta.PointersTest do
  ...
  describe "forge!" do
    ...
    test "forges a pointer for a peer" do
      peer = fake_peer!()
      pointer = Bonfire.Common.Pointers.forge!(peer)
    ...
    test "forges a pointer for a user" do
      user = fake_user!()
      pointer = Bonfire.Common.Pointers.forge!(user)
  ...
  describe "follow" do
    ...
    test "follows pointers" do
      Repo.transaction(fn ->
        ...
        assert pointer = Bonfire.Common.Pointers.one!(id: peer.id)
        IO.inspect(pointer)
        assert table = Bonfire.Common.Pointers.table!(pointer)
        assert table.table == "mn_peer"
        assert table.schema == Peer
        assert table.id == pointer.table_id
        peer = Map.drop(peer, [:is_disabled])
        assert peer2 = Bonfire.Common.Pointers.follow!(pointer)
    ...
    test "preload! can load many pointers" do
      Repo.transaction(fn ->
        ...
        assert pointer = Bonfire.Common.Pointers.one!(id: peer.id)
        assert pointer2 = Bonfire.Common.Pointers.one!(id: peer2.id)
        assert [pointer3, pointer4] = Bonfire.Common.Pointers.preload!([pointer, pointer2])
    ...
    test "preload! can load many pointers of many types" do
      Repo.transaction(fn ->
        ...
        assert pointer = Bonfire.Common.Pointers.one!(id: peer.id)
        assert pointer2 = Bonfire.Common.Pointers.one!(id: comm.id)
        assert pointer3 = Bonfire.Common.Pointers.one!(id: user.id)
        assert pointer4 = Bonfire.Common.Pointers.one!(id: comment.id)

        assert [pointer5, pointer6, pointer7, pointer8] =
                 Bonfire.Common.Pointers.preload!([pointer, pointer2, pointer3, pointer4])
